...
<head>
    ...
    <title>PMS &mdash; My Nurses</title>
...
<div class="flex min-h-screen">
    ...
    <main class="flex-1 ml-64">
        <header class="bg-white shadow-sm px-8 py-4 flex items-center justify-between sticky top-0 z-10">
            <div>
                <h1 class="text-xl font-bold text-gray-800">My Nurses</h1>
                <p class="text-gray-400 text-xs mt-0.5">Nurses registered under your supervision</p>
            </div>
                            ...
                            <c:choose>
                                <c:when test="${empty nurses}">
                                    <tr><td colspan="6" class="py-12 text-center text-gray-400">
                                        <i class="fas fa-user-nurse text-4xl mb-3 block opacity-30"></i>
                                        No nurses registered yet. <a href="${pageContext.request.contextPath}/doctor/add-nurse" class="text-teal-600 underline">Add one now.</a>
                                    </td></tr>
                                </c:when>
                                <c:otherwise>
                                        ...
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <span class="font-semibold text-gray-800">${n.firstName} ${n.lastName}</span>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4"><span class="bg-teal-50 text-teal-700 px-2 py-1 rounded-lg text-xs font-medium">${n.healthCenter}</span></td>
                                            <td class="px-6 py-4 text-gray-500">${n.email}</td>
                                            <td class="px-6 py-4 text-gray-500">${n.telephone}</td>
                                            <td class="px-6 py-4 text-gray-500 max-w-xs truncate">${n.address}</td>
                                        </tr>
                                    </c:forEach>
                                </c:otherwise>
                            </c:choose>
